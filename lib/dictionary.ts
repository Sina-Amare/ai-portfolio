...
export const dict = {
  ...
  fa: {
    ...
    about: {
      ...
      bio: "یه توسعه‌دهنده Python هستم با مدرک علوم کامپیوتر؛ تمرکزم روی بک‌اند و اپلیکیشن‌هاییه که با LLM کار می‌کنن. در کنارش با Mercor هم به شکل قراردادی روی ارزیابی مدل‌های AI کار می‌کنم — پرامپت‌ها و تسک‌هایی طراحی می‌کنم که نشون بدن مدل‌های پیشرفته دقیقاً کجا کم میارن. بیشتر با ساختن یاد می‌گیرم؛ پروژه‌های متن‌بازم همون‌جاییه که ایده‌ها رو محک می‌زنم و تا سطح یه محصول واقعی جلو می‌برم. هنوز اول مسیرم و در حال رشدم، ولی سر تحویل کار، تست نوشتن و تموم کردنش حسابی جدی‌ام.",
      strengthsLabel: "چی به کار اضافه می‌کنم",
      strengths: [
        "کار عملی با AI و LLM — ‏RAG، ‏LangGraph و جابه‌جایی خودکار بین چند سرویس، پشت FastAPI",
        "ارزیابی مدل‌های پیشرفته — پرامپت‌هایی که مدل رو به چالش می‌کشن، جواب‌های مرجع تأییدشده و rubricهایی که خطای مدل رو قابل تکرار می‌کنن",
        "بک‌اند مقاوم — کارها اجاره‌ای پردازش می‌شن و یه watchdog کارهای گیرکرده رو دوباره راه می‌ندازه؛ بعد از crash کار ادامه پیدا می‌کنه، نه اینکه از دست بره",
        "سریع از نمونه اولیه تا محصول نهایی، با تست و معماری تمیز",
      ],
      quote:
      ...
      experience: [
        {
          ...
          org: "Mercor",
          period: "۲۰۲۶ · اکنون",
          note: "ارزیابی سخت‌گیرانه مدل‌های زبانی پیشرفته — پرامپت‌ها و تسک‌هایی طراحی می‌کنم که عمداً مدل‌های قوی رو به خطا بندازن: توی جست‌وجوی وب، استدلال چندمرحله‌ای، و اون حالت‌هایی که مدل با اعتمادبه‌نفس کامل حرف اشتباه می‌زنه. برای هر تسک یه جواب مرجع تأییدشده و یه rubric تک‌جوابی می‌نویسم تا هر خطا قابل تکرار باشه و عینی نمره بگیره، نه سلیقه‌ای.",
        },
        {
          role: "توسعه‌دهنده نرم‌افزار",
          org: "Dekamond",
          period: "۲۰۲۵ · ۶ ماه",
          note: "ساخت بخش‌های AI و اتوماسیون برای Kaleri.ai — چندین سیستم خودکارسازی و pipeline جمع‌آوری داده نوشتم، جریان‌های کاری LLM با چند provider (LangGraph و RAG) پشت FastAPI برای ابزارهای تحلیل کسب‌وکار و ابزارهای داخلی تیم ساختم، و هزینه اجرای مدل‌ها رو با مسیریابی هوشمند بین مدل‌ها، cache و بهینه کردن پرامپت‌ها حدود ۷۰٪ کم کردم.",
        },
        {
          role: "توسعه‌دهنده Django (بک‌اند)",
          org: "Arnikup",
          period: "۲۰۲۴ · ۶ ماه",
          note: "ساخت و نگهداری REST API برای یه اپلیکیشن سفارش غذا توی یه تیم پنج‌نفره — مدل‌سازی محصولات و سفارش‌ها توی PostgreSQL با Django REST Framework، سپردن کارهای پس‌زمینه مثل پردازش سفارش و اعلان‌ها به Celery، استفاده از Redis برای cache و صف، کانتینری کردن سرویس‌ها با Docker و تست با pytest.",
        },
      ],
