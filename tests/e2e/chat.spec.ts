...
const happy = sse([
  {
    type: "data-sources",
    id: "sources",
    data: [{ source: "CV", section: "Summary" }],
  },
...
const refusal = sse([
  { type: "text-start", id: "0" },
  {
    type: "text-delta",
    id: "0",
    delta: "That's a little outside what I can chat about.",
  },
  { type: "text-end", id: "0" },
]);
...
test("happy path: streams a grounded answer with a source chip", async ({
  page,
}) => {
  await mockChat(page, happy);
  await page.goto("/#chat");
  await page
    .getByRole("button", { name: "What did you build at Dekamond?" })
    .click();
  await expect(
    page.getByText("I built RAG systems at Dekamond."),
  ).toBeVisible();
...
test("error path surfaces an alert with a retry", async ({ page }) => {
  ...
  await page.goto("/#chat");
  await page
    .getByRole("button", { name: "Are you open to new roles?" })
    .click();
  await expect(page.getByText("Something went wrong.")).toBeVisible();
  await expect(page.getByRole("button", { name: "Retry" })).toBeVisible();
});

test("language toggle switches the UI and suggestions to Persian", async ({
  page,
}) => {
  await page.goto("/#chat");
