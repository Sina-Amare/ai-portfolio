...
Production-minded open-source tools: ScrapeGPT (a self-hosted, AI-assisted web scraper where an LLM proposes the selectors and the app re-validates and self-heals them against the real HTML), Aigram (formerly SakaiBot — a self-hosted, installable AI messenger on your own Telegram account: read and send messages with inline media, with an AI co-pilot in every chat for ask-anything, translate, summarize, image generation, and voice, backed by API-key rotation and provider failover), RubricEval (a rubric-driven code-evaluation platform — repo github.com/Sina-Amare/github-code-review — where an LLM grades a submitted repo or ZIP against a versioned rubric but deterministic code makes the final accept / review / reject decision), and PromptAmp (a BYOK browser extension, live on Firefox Add-ons, that turns a rough draft into an engineered prompt inside any text field on any site).

## What is PromptAmp?
PromptAmp is Sina's browser extension — "the prompt amplifier: your keys, any site." A small button appears in any text field on any website; press it and your rough draft is rewritten into an engineered prompt using your own API key (OpenAI, Anthropic, Gemini, Groq, OpenRouter via one-click OAuth, local Ollama/LM Studio, or any OpenAI-compatible endpoint), shown next to the original, and inserted only when you accept. Pasted code or logs come back byte-for-byte — only the ask gets engineered — and drafts work in any language: the prompt comes back in the draft's language, or in any output language you choose from a list of fourteen (plus a custom option), so for example a Persian draft can become a polished English prompt. It's private by construction: no backend, no telemetry, keys stored only locally. The hard part under the hood is a six-tier insertion engine that writes through input-event paths so React- and ProseMirror-based editors (and your Ctrl+Z) keep working. It's live on Firefox Add-ons, with Chrome and Edge builds from the same WXT codebase.
