...
A real repository has far more code than fits in a single prompt, so the grader never dumps the whole repo at the model. For each criterion it sends a compact project-structure overview plus only the most relevant files — scored by structural role and the criterion's own signal terms, and bounded by a file-count and character budget. This keeps every call well under free-tier token limits and means the model grades the real application code, not just whatever happens to sort first.

## Built to run reliably
RubricEval is engineered like production software: ingestion guards against zip-slip and zip-bombs and against malicious git protocols; a durable leased job queue (Postgres FOR UPDATE SKIP LOCKED, with a SQLite fallback and boot-time orphan reclaim) means reviews survive crashes and scale across workers; results stream live to the browser over SSE with a durable, replayable event log (resumable via Last-Event-ID); a FakeLLM port lets the whole engine run deterministically offline and in CI; and a golden-set regression harness measures every prompt or model change instead of guessing. Every route except /health is behind operator-token auth (default-deny). On the provider side it tries the default model across all its (optionally comma-separated, rotated) keys, then each configured fallback model in turn, so one provider's quota or outage doesn't fail a review.
