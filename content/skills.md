...
## LLM evaluation and adversarial testing
Sina doesn't just build on language models — he evaluates and stress-tests them. As an AI training and evaluation contractor with Mercor (remote, since April 2026) he does adversarial evaluation of frontier LLMs: he designs prompts and evaluation tasks specifically built to make strong models fail, targeting web search and retrieval (making a model actually find and use a source rather than recall it), multi-step reasoning (where one wrong intermediate step quietly poisons the answer), and the edge cases where a model is confidently wrong. For each task he writes a verified golden answer and a single-answer rubric, so every failure is reproducible and objectively scorable rather than a matter of opinion. That discipline feeds directly back into what he builds: RubricEval grades code against a versioned rubric but hands the final accept / review / reject decision to deterministic code, and this portfolio's chatbot refuses out-of-scope questions instead of guessing. Knowing precisely how models break is what makes the systems he ships treat model output as something to verify, never to trust.
